...
const CartDrawer = () => {
  ...
  return (
    ...
    <div className={`fixed w-[98%] md:w-[800px] h-[calc(100vh-65px)] md:h-[100vh] pb-20 md:pb-60 top-[65px] md:top-[0px] right-0 bg-gray-200 md:bg-gray-50 shadow-lg z-[2000] transition-transform duration-1000 ease-in-out ${isCartOpen ? 'translate-x-0' : 'translate-x-[200%]'} overflow-y-scroll`}>
      ...
      <div className='w-full md:mt-6 px-[2%] md:px-4 flex flex-col gap-2'>
        {cart.length === 0 ? (
          <p className="mt-40 text-xl text-center text-gray-700">No tienes productos en tu carrito...</p>
        ) : (
          cart.map((item,i) => (
      ...
      {!user ? (
          <div className='mt-10 flex flex-col items-end px-4'>
            <p className='text-lg text-red-500'>Debes iniciar sesion para procesar la compra.</p>
